...
export function getLiveStats(): LiveStats {
  ...
  const wakaPublic = items.find(
    i => i.source === "wakatime" && i.id === "wakatime:rating:public-alltime"
  );
  if (wakaPublic) {
  ...
  if (lc) {
    ...
    const cal = lc.payload.calendar as { activeYears?: number[] } | undefined;
  ...
  // --- Credly ---
  const badges = items.filter(i => i.source === "credly" && i.kind === "badge");
  if (badges.length > 0) {
